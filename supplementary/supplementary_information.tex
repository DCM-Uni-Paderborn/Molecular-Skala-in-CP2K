...
{\color{blue}The companion repository \texttt{DCM-Uni-Paderborn/Molecular-Skala-in-CP2K} contains the molecular structures, \CPtwoK{} inputs, selected raw outputs, processed tables, checksums, and short extraction scripts needed to reproduce the numerical tables in the manuscript.  The Spark CPU/GPU timing diagnostic reported above is included there as raw \CPtwoK{} outputs, run summaries, and the processed table \texttt{processed/gpu\_timing\_summary\_20260524.tsv}.}  Diagnostic development files that are not required for reproducing the published tables are kept out of the Supplementary Information and, where retained for provenance, are clearly separated in the repository.
